...
namespace SharpNeat.EvolutionAlgorithms
{
	...
	public class MAPElitesEvolutionAlgorithm<TGenome> : AbstractGenerationalAlgorithm<TGenome>
        where TGenome : class, IGenome<TGenome>
	{
		...
		/// </summary>
		public MAPElitesEvolutionAlgorithm ()
		{
		...
		private SpecieStats[] CalcSpecieStats (out int offspringCount)
		{
			...
			if (0.0 == totalMeanFitness) {   // Handle specific case where all genomes/species have a zero fitness. 
			...
			if (0 == specieStatsArr [_bestSpecieIdx]._targetSizeInt) {
				...
				} else {   // Scan forward from this specie to find a suitable one.
					bool done = false;
					idx++;
			...
			for (int specieIdx = 0; specieIdx < specieCount; specieIdx++) {
				...
				if (1 == inst._selectionSizeInt) {   
					...
					for (; matingsCount < inst._offspringSexualCount; matingsCount++) {
						...
						if (0.0 != rwlTmp.ProbabilitiesTotal) {   // Get the two parents to mate.
							...
							offspringList.Add (offspring);
						} else {   // No other parent has a non-zero selection probability (they all have zero fitness).
