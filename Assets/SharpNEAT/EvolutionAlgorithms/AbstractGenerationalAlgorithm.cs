...
namespace SharpNeat.EvolutionAlgorithms
{
        ...
        where TGenome : class, IGenome<TGenome>
	{
		...
		protected uint _prevUpdateGeneration;
		protected long _prevUpdateTimeTick;

		// Misc working variables.
        
		bool _pauseRequestFlag;
		protected readonly AutoResetEvent _awaitPauseEvent = new AutoResetEvent (false);
		protected readonly AutoResetEvent _awaitRestartEvent = new AutoResetEvent (false);
		...
		public void StartContinueMAP2 ()
		{
			...
			if (RunState.Ready == _runState) {   // Create a new thread and start it running.
				//   print("RunState ready");
				_runState = RunState.Running;
				Coroutiner.StartCoroutine (AlgorithmThreadMethodMAP ());
				//   print("Continue from AlgorithmThreadMethod in StartContinue");
				OnUpdateEvent ();
		...
		{
			if (RunState.Running == _runState) {   // Set a flag that tells the algorithm thread to enter the paused state and wait 
